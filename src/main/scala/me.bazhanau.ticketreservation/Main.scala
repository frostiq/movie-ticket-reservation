...
import me.bazhanau.ticketreservation.dao.MovieTitleDbCache
...
object Main extends App with Routes with StrictLogging {
  ...
  val db = mongoClient.getDatabase(config.getString("mongodb.dbName"))
  val movieDao = new MongoMovieDao(db)

  val requester = Http().singleRequest(_: HttpRequest)
  val baseUri = Uri(config.getString("omdbapi.baseUrl"))
  val movieTitleDao: MovieTitleDao = new MovieTitleDbCache(
    new MovieTitleWebDao(requester, baseUri, config.getString("omdbapi.apiKey")),
    db
  )
